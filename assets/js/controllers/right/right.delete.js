+App.controller('RightDeleteCtrl', function ($scope, Right, $state, $stateParams, Restangular, toastr, RightConfig, principal) {
  $scope.title = 'Xóa thông tin nhóm quyền';
...
// /Panel toolbox
  ...
  $scope.deleteRightGroup = function() {
		Right.one().one('delete-right-group').remove({id: $stateParams.id}).then(function (result) {
			toastr.success('Xóa nhóm quyền thành công', 'Thông báo!');
			$state.go('right.list');
		}, function (err) {
			console.log(err);
			toastr.error('Không thể xóa nhóm quyền do có tài khoản hoạt động trực thuộc!', 'Lỗi!');
		});
	};
