+App.controller('RightEditCtrl', function ($scope, Right, $state, $stateParams, Restangular, toastr, RightConfig, principal) {
...
// /Panel toolbox
  ...
  // console.log('groupName before: ' + $scope.information.groupName);
  ...
  //kiem tra User active trong nhom quyen
  Right.one('get-right-group-by-id-for-user').get({id: $stateParams.id}).then(function (result) {
    for (var i = 0; i < result.data[0].roleId_user.length; i++) {
      if (result.data[0].roleId_user[i].status == 1) {
        checkUserActive = 1;
      }
    }
    console.log('Length: ' + result.data[0].users.length);
  });

  var tmpArr = [];
  $scope.updateRight = function () {
    getIDOfEdit($scope.selected1);
    getIDOfEdit($scope.selected10);
    getIDOfEdit($scope.selected13);
    getIDOfEdit($scope.selected17);
    getIDOfEdit($scope.selected23);
    getIDOfEdit($scope.selected29);

    if ($scope.rightEditForm.$invalid) {
      angular.forEach($scope.rightEditForm.$error, function (field) {
    ...
    if ($scope.information.status == 0 && checkUserActive == 1) {
      toastr.error('Không được ngừng hoạt động nhóm quyền do có tài khoản đang hoạt động!', 'Lỗi!');
      return false;
    }

    $scope.information.features = tmpArr;
    if (($scope.information.features).length == 0) {
      toastr.error('Vui lòng chọn ít nhất 1 quyền!', 'Lỗi!');
      return false;
    }
    $scope.information.id = $stateParams.id;
    Restangular.one('right/update-right-group').customPUT($scope.information).then(function (data) {
